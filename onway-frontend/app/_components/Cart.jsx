...
function Cart({ cart }) {
  ...
  return (
    <div className=" max-w-4xl mx-auto ">
      <h2 className="font-semibold  text-lg md:text-lg">My Order</h2>
          ...
          <table className="w-full border-collapse">
            ...
            <tbody>
              {cart?.items.map((item, index) => (
                <tr key={index} className="border border-gray-300 items-center">
      ...
      <Link href={`/checkout?business=${cart?.items[0]?.restaurant?.slug}`} className="block w-full mt-4">
        <button className="bg-green-800 text-white p-3 text-lg w-full rounded-lg shadow-md">
